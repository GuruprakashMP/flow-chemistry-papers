...
*continuous-flow / flow chemistry is its primary subject* — any methodology
(synthesis, reactor engineering, in-line analytics, modelling) qualifies, but
neighbouring fields that merely share the word "flow" (flow cytometry, blood
flow, CFD/heat-transfer engineering, traffic/hydrology, nuclear reactors) are
rejected.
...
# Minimum evidence that flow chemistry is the primary subject: one
# unambiguous term ("flow chemistry", "microreactor", "telescoped
# synthesis"...) suffices; weak circumstantial matches alone do not.
...
def _compile(vocab: Dict[str, object]) -> List[Tuple[re.Pattern, str]]:
    ...
    "microreactor" matches "microreactor" / "microreactors"; "telescoping"
    matches "telescoping(s)".
    """
    compiled = []
    for phrase in vocab:
...
def classify(record: RawRecord) -> Classification:
    ...
    if prim_pts < _MIN_PRIMARY_POINTS:
        return Classification(False, 0, reason="no flow chemistry evidence")
    ...
    # paper reads as "flow chemistry topic + supporting details".
    ...
    if "Continuous Flow Synthesis" in categories and len(categories) > 1:
        categories.remove("Continuous Flow Synthesis")
